...
def generate_myntra_embeddings(
    articles: pd.DataFrame,
    model: SentenceTransformer = None,
    top_n: int = 10_000,
    batch_size: int = 64,
    ...
    """
    Generate dense vector embeddings for the top N most popular Myntra articles.

    Returns:
        embeddings: np.ndarray of shape (N, 384)
        catalog: The filtered article DataFrame with matching index
    """
    emb_path = os.path.join(PROCESSED_DIR, "myntra_catalog_embeddings.npy")
    catalog_path = os.path.join(PROCESSED_DIR, "myntra_catalog.parquet")

    # Use cached embeddings if available
    if os.path.exists(emb_path) and os.path.exists(catalog_path):
        print("[Embeddings] Loading cached Myntra embeddings...")
        embeddings = np.load(emb_path)
        catalog = pd.read_parquet(catalog_path)
        return embeddings, catalog
    ...
    # Ensure product_url is included in catalog for Myntra
    if 'product_url' in articles.columns:
        catalog['product_url'] = catalog['product_url']
    else:
        catalog['product_url'] = ''  # Empty string if no URLs available

    texts = catalog["semantic_text"].tolist()
    print(f"[Embeddings] Encoding {len(texts):,} Myntra articles (batch_size={batch_size})...")
    embeddings = model.encode(texts, batch_size=batch_size, show_progress_bar=True)

    # Cache to disk
    np.save(emb_path, embeddings)
    catalog.to_parquet(catalog_path, index=False)
    print(f"[Embeddings] Saved Myntra embeddings {embeddings.shape} -> {emb_path}")
...
if __name__ == "__main__":
    ...
    print("Myntra Embeddings shape:", embeddings.shape)
